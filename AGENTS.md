...
3. No hardcoded URLs, paths, or service bases. Use `lib/constants.ts` / `lib/paths.ts` / `lib/runtime-env.ts` and document canonical values in `knowledge/paths.md`.
...
6. No MUI and no `@msqdx/react` for new or migrated surfaces.
