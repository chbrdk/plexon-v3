...
- Suite Enterprise: activity `…/activity` · audit `…/audit` · campaign briefs `…/campaign-briefs` · share-links `…/share-links` (`apiPlatformProvisioningCollectionShareLinks`, spec `collection-share-links.md`; Hub revoke fans out to Checkion/Metron provisioning DELETE) · client room `…/client-room` (API remains; Collection-Home UX deferred) · slot put `…/client-room/slots/{slotId}` · directory stub `GET/PATCH /api/admin/companies/:id/directory` · knowledge `knowledge/suite-enterprise-program.md` · slot contract `knowledge/client-room-slots.md`
- Use-case testing playbook: `knowledge/suite-use-case-testing.md` · Playwright `e2e/` (UC1, UC9+E1/E4 APIs, E3, E5/E8 templateId regression, E9 stub) · Env keys only (no values in git): `E2E_BASE_URL` (default staging plexon-v3 FQDN) · `E2E_USER` · `E2E_PASSWORD` · local file `.env.e2e.local` (gitignored) · script `pnpm test:e2e:staging` / `npm run test:e2e:staging` · install browsers once: `npx playwright install chromium`
