...
- Outbox background drain: `instrumentation.ts` (Node-only + `webpackIgnore`) + `lib/platform-outbox-scheduler.ts` (env `PLEXON_OUTBOX_DRAIN_ENABLED` / `PLEXON_OUTBOX_DRAIN_INTERVAL_MS`, default 30s)
