...
- METRON (Wave 1 AppShell · capability Phase 8 planned): product id `metron` · repo `https://github.com/chbrdk/metron-v3` · local `http://localhost:3011` · env stubs `NEXT_PUBLIC_METRON_URL` / `METRON_API_URL` / `METRON_MCP_URL` · fit `knowledge/metron-ecosystem-fit.md` · onboarding `knowledge/metron-v3-onboarding.md` · domain `specs/domain/metron-capability.md` · FQDN TBD after Coolify smoke · launch `{METRON}/projects?platformProjectId=` · origin `POST /api/platform/provisioning/metron-project-origin` (not landed — Wave 2)
