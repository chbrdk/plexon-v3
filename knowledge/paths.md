...
- METRON (Wave 5 + staging): product id `metron` · repo `https://github.com/chbrdk/metron-v3` · local `http://localhost:3011` · staging `https://metron-v3.projects-a.plygrnd.tech` · Coolify `8qkrk850d37er6subakpnx0r` · env `NEXT_PUBLIC_METRON_URL` / `METRON_API_URL` / `METRON_MCP_URL` · fit `knowledge/metron-ecosystem-fit.md` · onboarding `knowledge/metron-v3-onboarding.md` · domain `specs/domain/metron-capability.md` · launch `{METRON}/projects?platformProjectId=` · origin `POST /api/platform/provisioning/metron-project-origin` · upsert `{METRON}/api/platform/provisioning/projects/{platformProjectId}`
...
- SPIRION MCP (design references/screens): `SPIRION_MCP_URL` → staging `https://spirion-api.projects-a.plygrnd.tech/mcp` (legacy `DIG_MCP_URL`) · spec `specs/domain/assistant-spirion-mcp.md` · `knowledge/spirion-mcp-assistant.md` · Welle 2 campaign/graphic corpus `specs/domain/spirion-campaign-motif-corpus.md` · `knowledge/spirion-campaign-motif-corpus.md` · tickets `knowledge/spirion-campaign-motif-tickets.md` · craft module `campaign_motif_ref_v1` (`knowledge/creation-craft-module-campaign-motif-ref.md`) · Dribbble connector env lives on dig-api (`DRIBBBLE_CLIENT_ID` / `DRIBBBLE_CLIENT_SECRET`), not in plexon app code
...
- METRON MCP (KPIs/dashboards): `METRON_MCP_URL` → staging `https://hh0pad7nwoupxnydpd7shb9r.projects-a.plygrnd.tech` · Coolify `metron-mcp` `hh0pad7nwoupxnydpd7shb9r` port **3104** · spec `specs/domain/assistant-metron-mcp.md` · `knowledge/metron-mcp-assistant.md` · product `metron-v3/mcp-server/`
- METRON public dashboard share: `/share/metron/:token` · magazine chrome (read-only + copy + print/PDF) · `POST /api/assistant/metron/dashboards/share` · `GET /api/public/metron/:token` · spec `specs/domain/assistant-metron-share.md` · constants `PATH_SHARE_METRON` / `pathShareMetron` / `apiPublicMetron` / `apiAssistantMetronDashboardShare`
- Assistant Collection deep-link: `pathAssistantWithProject` · optional draft seed `ASSISTANT_DRAFT_PROMPT_QUERY_PARAM` (`draft`) via `pathAssistantWithProjectAndDraft` (METRON Collection tile “Ask in Assistant”)
- Expand: `PATH_ASSISTANT` = `/assistant` · `pathAssistantChat` / `pathAssistantWithProject` / `pathAssistantWithProjectAndDraft`
...
- CREATION MCP (library/compositions/scene): `CREATION_MCP_URL` → attach `creation-mcp` port **3102** · spec `specs/domain/assistant-creation-mcp.md` · `knowledge/creation-mcp-assistant.md` · product MCP `creation-v3/mcp-server/` · tools include v2 scene + `creation.scene_craft_debug` + `creation.site_kit_page_save` · domain autonomy wave `specs/domain/assistant-creation-agi-lite.md` · `knowledge/assistant-creation-agi-lite.md` · craft playbooks (web/newsletter/print) `knowledge/creation-craft-playbooks.md` · craft modules `lib/assistant/creation-craft-modules.ts` · Spirion meta `knowledge/creation-craft-module-spirion-section-ref.md` · craft memory `lib/assistant/knowledge-pack/distill-creation-craft.ts` (`ASSISTANT_CREATION_CRAFT_MEMORY`) · Wave D eval/routing `knowledge/creation-craft-eval.md` (`ASSISTANT_CREATION_MODEL_TIER` · `ASSISTANT_CREATION_EVAL_MODE` · optional `ASSISTANT_CREATION_HIGH_MODEL`) · print report atom wave (agent Phase E) `knowledge/print-report-primitives-agent.md` (DS `msqdx-ui` P92 · Creation consume)
- AUDION MCP: `AUDION_MCP_URL` → staging `https://mcp-audion.projects-a.plygrnd.tech` (`audion-mcp`)
- AUDION binding heal (resolve-before-create): `knowledge/audion-binding-heal.md` · `lib/assistant/workflows/heal-audion-binding.ts` · ensure via `ensurePlatformProductBindings` · origin rebind `POST /api/platform/provisioning/audion-project-origin` (+ optional `platformProjectId`) · Audion GET `{AUDION}/api/platform/provisioning/projects/{platformProjectId}`
...
- Collection invite links (Access Model B): Spec `specs/domain/collection-invite-links.md` · API `specs/api/collection-invites.md` · page `/invite/:token` · create/list/revoke `/api/platform/provisioning/collections/:platformProjectId/invites` · accept `POST /api/platform/invites/:token/accept` · table `collection_invites` · helpers `lib/collection-invites.ts` · constants `PATH_COLLECTION_INVITE` / `apiPlatformProvisioningCollectionInvites` · optional create `toEmail` → transactional mail (`specs/domain/transactional-email.md`)
- Transactional email (SMTP control plane): Spec `specs/domain/transactional-email.md` · API `specs/api/transactional-email.md` · knowledge `knowledge/transactional-email.md` · mailer `lib/mail/` · HTML shell `@msqdx/ui` `renderMsqdxEmailDocument` + `MSQDX_EMAIL_LOGO_MARK_URL` (`msqdx-ui/specs/domain/msqdx-ui-email.md` · `msqdx-ui/knowledge/email.md`) · facade `lib/send-password-reset-email.ts` · forgot `/forgot-password` · `API_AUTH_REQUEST_PASSWORD_RESET` · health `passwordResetMail` + `transactionalMail` · Coolify SMTP `knowledge/coolify-env-variablen.md` · **HTTPS bridge** `https://smtp_http_bridge-90fzj0soeu4ruawzq4l3xanx.plygrnd.tech` (Coolify `plexon-smtp-http-bridge`) via `PLEXON_SMTP_HTTP_URL` + `PLEXON_SMTP_HTTP_TOKEN` + `PLEXON_SMTP_HTTP_INSECURE_TLS` (transport `smtp_http`)

- CREATION Client Page Share (enterprise view-only for customers — **not** Collection invite): Spec `specs/domain/creation-client-share.md` · companion `creation-v3/specs/domain/client-page-share.md` · eval `creation-v3/knowledge/client-page-share-eval.md` · Creation hosts `/share/p/:token` + `crs_` tokens; Plexon owns org/Collection `clientShare` policy · P2 APIs `GET/PATCH …/client-share-policy` · `GET/POST …/client-shares` · `DELETE …/client-shares/:shareId` · **P4** Collection UI `components/projects/CollectionClientSharesPanel.tsx` on `/projects/:id` · revoke fans out to Creation provisioning DELETE · **P5** audit `POST …/client-share-events` · CSV `GET …/client-share-events/export` · **P6** company defaults `GET/PATCH /api/admin/companies/:id/client-share-policy` · `CompanyClientSharePolicyPanel` · merge in `getClientSharePolicy` · constants `apiAdminCompanyClientSharePolicy` · migrations `0017`–`0019` · knowledge `knowledge/creation-client-share.md`
- Collection members roster (Access Model B): Spec `specs/api/collection-members.md` · `GET/POST /api/platform/provisioning/collections/:platformProjectId/members` · `DELETE …/members/:userId` · helpers `lib/collection-members.ts` · constants `apiPlatformProvisioningCollectionMembers` · ecosystem UI consumers: `knowledge/collection-team-ecosystem.md` · on `status: added` → `collection_member_added` mail (P1)
...
- List Collections the user can see (same directory as Sync; default page 50, cursor pagination): `GET /api/platform/me/accessible-collections?limit=&cursor=` · service BFF: `GET /api/platform/provisioning/accessible-collections` (+ `X-Plexon-User-Id`) — CREATION P71 picker · product project lists (access model B: creator / assignment, not company-wide) — `knowledge/access-model-b-project-visibility.md` · Wave B `specs/domain/collection-read-model.md`
- Collection projection (read model): `GET/POST /api/platform/projects/{id}/projection` · `apiPlatformProjectProjection` · rebuild on pack/binding writes
- Data-plane ops: `GET /api/platform/ops/data-plane` · outbox drain `POST /api/platform/ops/outbox/drain` · facet freshness `POST …/knowledge/facets/:facetId/freshness` (`apiPlatformProjectKnowledgeFacetFreshness`) · program `knowledge/plexon-data-plane-optimizations.md`
...
- Suite progress checkpoint (SSOT „wo stehen wir?“): `knowledge/suite-stand.md` · Enterprise detail `knowledge/suite-enterprise-program.md` · slot contract `knowledge/client-room-slots.md`
- Suite Cleanup network: `knowledge/suite-cleanup.md` · Freigabe `knowledge/suite-cleanup-drop-safe.md` · skill `.cursor/skills/suite-cleanup` · per-repo `knowledge/cleanup-inventory.md` + `keep-drop-backlog.md`
- Suite Enterprise routes: activity `…/activity` · audit `…/audit` · campaign briefs `…/campaign-briefs` · share-links `…/share-links` (`apiPlatformProvisioningCollectionShareLinks`, spec `collection-share-links.md`; Hub revoke fans out to Checkion/Metron provisioning DELETE) · client room `…/client-room` (API remains; Collection-Home UX deferred) · slot put `…/client-room/slots/{slotId}` · directory stub `GET/PATCH /api/admin/companies/:id/directory`
- Use-case testing playbook: `knowledge/suite-use-case-testing.md` · Playwright `e2e/` (UC1, UC9+E1/E4 APIs, E3, E5/E8 templateId regression, E9 stub) · Env keys only (no values in git): `E2E_BASE_URL` (default staging plexon-v3 FQDN) · `E2E_USER` · `E2E_PASSWORD` · local file `.env.e2e.local` (gitignored) · script `pnpm test:e2e:staging` / `npm run test:e2e:staging` · install browsers once: `npx playwright install chromium` · last full green: 2026-09-26 (13/13)
- E2E home bootstrap (session): `POST /api/platform/me/bootstrap-home` → company + optional first Collection (`API_PLATFORM_ME_BOOTSTRAP_HOME`) — called from Playwright after login
...
- Enterprise SQL boot apply: `scripts/apply-enterprise-sql-migrations.mjs` (0020/0021/0022) after drizzle push
...
- Central Assistant Flyout: `specs/domain/central-assistant-flyout.md` · `specs/api/assistant-embed.md` · `knowledge/central-assistant-flyout.md` · first-turn create SSOT `lib/assistant/resolve-assistant-conversation.ts` (omit `conversationId` → mint; unknown id → 404) · continuity beacon `POST /api/assistant/continuity` (`API_ASSISTANT_CONTINUITY`)
- Assistant actor identity (session user for product MCP/RAG): `specs/domain/assistant-actor-identity.md` · `knowledge/assistant-actor-identity.md`
  ...
  - Expand: `PATH_ASSISTANT` = `/assistant` · `pathAssistantChat` / `pathAssistantWithProject` / `pathAssistantWithProjectAndDraft` (`ASSISTANT_DRAFT_PROMPT_QUERY_PARAM`)
...
- Collection projects: `specs/domain/collection-projects.md`
  ...
  - Capability Catalog (Agent ↔ Flow, Wave C4): `specs/domain/capability-catalog.md` · `knowledge/capability-catalog.md` · executors `lib/capabilities/executors/*` · promote `lib/capabilities/promote.ts` · intents `run_collection_flow` / `promote_capability_sequence` · trigger `assistant` · template `assistant-promote-v1` · tests `__tests__/capability-*.test.ts` · env `CAPABILITY_CATALOG_RUNTIME` (code default **off**; staging Coolify **on**) · health field `capabilityCatalogRuntime`
...
- Collection Test Flow Wave 4 (Study rollup):
  ...
  - Wave 1 Collection Memory: `specs/domain/collection-memory-wave1.md` · Vaillant `vaillant-uc*-flow-latest` · Assistant `assistant-report-latest` · Creation craft `creation-craft-prefs-latest` / `creation-*-recipe-latest` · `lib/demo/vaillant-flow-knowledge-distillate.ts` · `lib/assistant/knowledge-pack/distill-assistant-report.ts` · `lib/assistant/knowledge-pack/distill-creation-craft.ts`
