...
- Transactional email (SMTP control plane): Spec `specs/domain/transactional-email.md` · API `specs/api/transactional-email.md` · knowledge `knowledge/transactional-email.md` · mailer `lib/mail/` · facade `lib/send-password-reset-email.ts` · forgot `/forgot-password` · `API_AUTH_REQUEST_PASSWORD_RESET` · health `passwordResetMail` + `transactionalMail` · Coolify SMTP `knowledge/coolify-env-variablen.md` · **HTTPS bridge** Coolify service `plexon-smtp-http-bridge` FQDN `https://smtp_http_bridge-90fzj0soeu4ruawzq4l3xanx.plygrnd.tech` (`PLEXON_SMTP_HTTP_URL` + `PLEXON_SMTP_HTTP_TOKEN`; transport `smtp_http`)
