...
- **Konvertierung:** `lib/usage-conversion.ts` – z. B. `llm_request` → input_tokens + 2×output_tokens (auch `prompt_tokens`/`completion_tokens`); `scan_wcag` → 50×scans; `scan_pagespeed` → 20; CHECKION **`domain_scan_page`** → 50×pages (pro abgeschlossener Deep-Scan-Seite, `raw_units` mit `domain_scan_id`, `page_index`, `ok`, `url`); bei **`reused_unchanged: true`** (unveränderte Seite per HEAD/Reuse) → **5×pages**; `page_classify` (Legacy) → 40×pages oder max(40×pages, input+2×output) wenn Token-Felder gesetzt; `domain_scan` (Sammel) → 50×pages; `ux_check` / `serp_refresh`; CHECKION-Tools `tool_extract` (28), `wayback_lookup` (6), `ssl_labs_analyze` (18) pro Request; CHECKION Market SEO **`seo_dataforseo`** → `cost_usd × 100_000` (Vendor-USD ≈ Plexon-Einheiten); generisches **`vendor_cost`** (OpenRouter/Jev USD) → gleiche `cost_usd × 100_000`-Anker; AUDION `journey_validate` → 35×personas; `persona_discover` → 75×runs; `retrieval_query` → 18×queries; **BRANDION** `brandion_detect` → PDF **40×pages** / Image **25×runs**; `brandion_measure` → **30×runs**; plus optional `llm_request` wenn Vision.
