...
| `CAPABILITY_CATALOG_RUNTIME` | Nein | Capability Catalog Agent↔Flow shared executors (Wave C1). Code-Default **off**. Explicit on: `1` / `true` / `on` / `yes`. **Staging plexon-v3:** `1` (Coolify). Visible on `GET /api/health` as `capabilityCatalogRuntime`. Spec: `specs/domain/capability-catalog.md`. |
