...
export function planAssistantTurnHeuristic(input: PlannerInput): AssistantPlan {
  ...
  if (VIDEON_PATTERNS.some((p) => p.test(text)) && input.hasVideonMcp) {
    return buildPlan({
      intent: 'videon_media',
      mode: 'tools',
...
export function buildPlanSystemPromptBlock(plan: AssistantPlan): string {
  const writeToolsNote = plan.allowWriteTools
    ? plan.intent === 'creation_scene_edit'
      ? '\n- Schreib-Tools aktiv: creation_scene_apply_ops und creation_scene_import_html (baseUpdatedAt aus Seitenkontext / vorherigem Tool-Result!). Neue Seite → add_page {name?} zuerst, dann unter neuem root.id. Insert: bevorzugt insert_child MIT props (echte CTAs/Options/Texte). insert_instance nur mit props oder set_prop im selben Batch — nie nackte Instances mit Seed-Copy (Get started / Option A / Text). Niemals insert_node/add_instance/append_child. Bei op-rejected/stale-scene die Server-Felder reason, failedIndex und scene.updatedAt zitieren und Retry mit frischem baseUpdatedAt — nicht raten.'
