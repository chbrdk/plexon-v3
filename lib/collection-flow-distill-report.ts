/**
 * Flow curated report → Collection Knowledge Pack.
...
export const FLOW_REPORT_KP_SECTION_ID = 'flow-report-latest' as const

export function buildFlowReportKnowledgeSection(input: {
  reportId: string
  flowId: string
  ...
  return {
    id: FLOW_REPORT_KP_SECTION_ID,
    title: input.narrative.title.slice(0, 200) || 'Flow report',
    plainText: [
...
export async function distillFlowReportToKnowledgePack(input: {
    ...
    const result = await patchKnowledgePackFacet({
      ...
      facetDocument: {
        ...
        provenance: {
          ...
          productId: 'plexon',
          note: 'flow report Wave 26 distillate',
