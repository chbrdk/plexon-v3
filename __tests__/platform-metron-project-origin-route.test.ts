...
describe('POST /api/platform/provisioning/metron-project-origin', () => {
  beforeEach(() => {
    vi.resetAllMocks();
    ...
    vi.mocked(syncPlatformProjectToProducts).mockResolvedValue([
      ...
      },
      {
        ...
        productId: 'spirion',
        ok: true,
        externalProjectId: 'spirion-99',
      },
    ]);
  });

  it('returns 401 without service secret', async () => {
    const { POST } = await import('@/app/api/platform/provisioning/metron-project-origin/route');
    const res = await POST(
      new Request('http://localhost/api/platform/provisioning/metron-project-origin', {
        method: 'POST',
  ...
  it('returns 201 and sibling ids on success', async () => {
    ...
    const res = await POST(
      new Request('http://localhost/api/platform/provisioning/metron-project-origin', {
        ...
        headers: authHeaders(),
        body: JSON.stringify({
          metronProjectId: 'met-1',
          name: 'Metron Project',
          domain: 'metron.example',
        }),
      })
    );
    ...
    expect(syncPlatformProjectToProducts).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        onlyProducts: expect.arrayContaining([
          'checkion',
          'audion',
          'brandion',
          'creation',
