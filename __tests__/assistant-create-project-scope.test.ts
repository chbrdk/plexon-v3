...
describe('create-project-scope (collection phase 1)', () => {
  it('always targets Collection platform (even when audion is named)', () => {
    expect(
      detectCreateProjectTarget(
        'kannst du ein neues projekt rheinland versicherungen in audion anlegen',
      ),
    ).toBe('platform')
    expect(detectCreateProjectTarget('LEGE es nur in audion an!!!!')).toBe('platform')
    expect(detectCreateProjectTarget('Lege ein neues Projekt "Acme" an')).toBe('platform')
    expect(detectCreateProjectTarget('Projekt Foo in audion und checkion anlegen')).toBe(
      'platform',
    )
  })
...
describe('intent-router collection create', () => {
  it('routes audion-scoped create to Collection create_project', () => {
  ...
  it('routes nur audion follow-up to Collection create_project', () => {
    expect(routeAssistantIntent('LEGE es nur in audion an!!!!').type).toBe('create_project')
  })
