...
describe('collection projects domain (phase 0)', () => {
  ...
  it('phase 5 documents archive/restore and admin hard-delete', () => {
    ...
    expect(spec).toContain('provisioning/projects/:platformProjectId')
    expect(spec).toContain('userCanManageCollectionLifecycle')
    expect(constants).toContain('apiPlatformProject')
    expect(constants).toContain('apiPlatformProvisioningProject')
