...
/**
 * Same-origin muted preview proxy for assistant video_hit_strip.
 * Forwards to VIDEON GET /api/media/:id/preview with service secret + session actor.
 */
export async function GET(request: NextRequest) {
  try {
    ...
    if (durationRaw != null && durationRaw.trim() !== '') params.set('durationMs', durationRaw.trim())
    const upstream = `${base}/api/media/${encodeURIComponent(mediaAssetId)}/preview?${params.toString()}`
        ...
        res.status === 401 || res.status === 403
          ...
          : res.status === 404
            ...
            : res.status === 409
              ? 409
              : 502
      return apiError(`VIDEON preview failed (${res.status})`, status)
    }
