...
const bodySchema = z.object({
  metronProjectId: z.string().min(1),
  name: z.string().min(1),
...
export async function POST(request: Request) {
  ...
  const metronId = parsed.metronProjectId.trim();

  const existingPlatformId = await findPlatformProjectIdByProductExternal('metron', metronId);
  if (existingPlatformId) {
    ...
    const siblings = await bestEffortSiblingMirrors(
      existingPlatformId,
      'plexon-metron-project-origin-idempotent'
    );
    return platformJson({
  ...
  try {
    ...
    return handleApiError(e, { context: 'metron-project-origin' });
  }
}
